...
export default function Authorization() {
  ...
  const columns = [
    { title: 'ID', dataIndex: 'id', key: 'id' },
    {
      ...
      key: 'avatar_url',
      render: (url: string) => (
        <img src={url || '/images/avatar-default.png'} alt="avatar" style={{ width: 40, height: 40, borderRadius: '50%', objectFit: "cover" }} />
      ),
    },
    ...
    { title: 'SĐT', dataIndex: 'phone', key: 'phone' },
    {
      ...
      dataIndex: 'role',
      key: 'role',
      render: (role: string) => <Tag color={role === 'admin' ? 'red' : role === 'staff' ? 'blue' : 'green'}>{role}</Tag>,
    },
    {
      title: 'Trạng thái',
      ...
      key: 'is_deleted',
      render: (val: string) => (
        <span className={`inline-block px-3 py-1 text-xs font-medium rounded-full ${val === 'active' ? 'bg-blue-100 text-blue-700' : 'bg-red-100 text-red-700'}`}>
