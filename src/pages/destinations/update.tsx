...
export default function EditDestination() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const highlightSec = sections.find((s: any) => s.type === 'highlight');
        const gallerySec = sections.find((s: any) => s.type === 'gallery');
        const delicaciesSec = sections.find((s: any) => s.type === 'regionalDelicacies');
        const experienceSec = sections.find((s: any) => s.type === 'experience');
        if (experienceSec) setExperience(experienceSec.description || '');
        const lastImageSec = sections.find((s: any) => s.type === 'lastImage');
        ...
        if (gallerySec) {
          const files = (gallerySec.content || []).map((url: string, idx: number) => ({
            uid: `${idx}`,
        ...
        if (delicaciesSec) {
          ...
          setDelicaciesDishes(dishes.map((d: any) => ({ name: d.name, image: d.image })));
  ...
  const onFinish = async (values: any) => {
    ...
    try {
      ...
      // 👉 Section: highlight
      const validHighlights = highlight.filter(h => h.title && h.description);
      ...
      notifyError(error?.response?.data?.message || 'Cập nhật điểm đến thất bại!');
      console.error(error);
    } finally {
      setLoading(false);
    }
      ...
      <div className="mx-auto bg-white p-8 shadow-[8px] rounded-md">
        ...
        <Form
          ...
          onFinishFailed={({ errorFields }) => {
            ...
            notifyError('Vui lòng điền đầy đủ các trường bắt buộc!');
          }}
          className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4"
        >
          <Form.Item label="Tên điểm đến" name="name" rules={[{ required: true }]}>
            <Input />
          </Form.Item>
          <Form.Item label="Khu vực" name="area" rules={[{ required: true }]}>
            <Input />
          </Form.Item>
          <Form.Item label="Danh mục điểm đến" name="category_id" rules={[{ required: true }]}>
          ...
          <Form.Item label="Mô tả" name="description" className="md:col-span-2" rules={[{ required: true, message: 'Vui lòng nhập mô tả điểm đến' }]}>
            <TextArea rows={4} />
          </Form.Item>

          <Divider className="md:col-span-2">Nội dung chi tiết</Divider>

          <Collapse className="md:col-span-2" defaultActiveKey={['intro', 'highlight']}>
              ...
              {highlight.map((item, idx) => (
                <Space key={idx} direction="vertical" style={{ display: 'block' }}>
                  <Input
                    ...
                    onChange={(e) =>
                      setHighlight((prev) =>
                        prev.map((h, i) => (i === idx ? { ...h, title: e.target.value } : h))
                      )
                    }
                  />
                  <TextArea
          ...
          <Form.Item className="md:col-span-2">
            <Space>
              ...
              <Button onClick={() => navigate('/destinations')}>Hủy</Button>
            </Space>
          </Form.Item>
        </Form>
      </div>
