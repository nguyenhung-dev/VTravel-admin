...
export default function CreateTour() {
  ...
  const fetchGuides = async () => {
    try {
      ...
      setGuides(res.data.filter((g: GuideType) => g.is_deleted === 'active'));
    } catch (error) {
  ...
  const fetchBusRoutes = async () => {
    try {
      ...
      setBusRoutes(res.data.filter((r: BusRouteType) => r.is_deleted === 'active'));
    } catch (error) {
  ...
  const onFinish = async (values: any) => {
    try {
      ...
      if (values.discount_price !== undefined && values.discount_price !== null && values.discount_price !== '') {
      ...
      if (values.schedules && Array.isArray(values.schedules)) {
        values.schedules.forEach((schedule: TourSchedule, index: number) => {
          Object.entries(schedule).forEach(([key, value]) => {
            if (value !== undefined && value !== null) {
              // Format thời gian thành H:i
              if (key === 'start_time' || key === 'end_time') {
                const time = new Date(`1970-01-01T${value}`);
                value = time.toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit', hour12: false });
              }
              formData.append(`schedules[${index}][${key}]`, String(value));
            }
          });
        });
      }
        ...
        >
          ...
          <Form.Item
            ...
            rules={[
              ...
              { max: 255, message: 'Tên tour không được vượt quá 255 ký tự' },
            ]}
          >
          ...
          <Form.Item
            ...
            rules={[{ required: true, message: 'Vui lòng chọn thời lượng tour!' }]}
          >
            <Select placeholder="Chọn thời lượng">
              ...
              <Select.Option value="5 ngày 4 đêm">5 ngày 4 đêm</Select.Option>
            </Select>
          </Form.Item>
          ...
          <Form.Item
            ...
            rules={[{ type: 'array', message: 'Vui lòng chọn ít nhất một điểm đến' }]}
          >
            <Select
              mode="multiple"
          ...
          <Form.Item label="Hướng dẫn viên" name="guide_id">
            <Select allowClear placeholder="Chọn hướng dẫn viên">
              {guides.map((g) => (
                <Select.Option key={g.guide_id} value={g.guide_id}>
                  {g.name}
                </Select.Option>
              ))}
            </Select>
          ...
          <Form.Item label="Tuyến xe buýt" name="bus_route_id">
            <Select allowClear placeholder="Chọn tuyến xe buýt">
              {busRoutes.map((route) => (
                <Select.Option key={route.bus_route_id} value={route.bus_route_id}>
                  {route.name}
                </Select.Option>
              ))}
            </Select>
                  ...
                  >
                    <Form.Item
                      ...
                      rules={[
                        ...
                        ({ getFieldValue }) => ({
                          validator(_, value) {
                            const days = getFieldValue('schedules').map((s: TourSchedule) => s.day);
                            if (days.filter((d: number) => d === value).length > 1) {
                              return Promise.reject('Ngày không được trùng lặp');
                            }
                    ...
                    >
                      <Input type="number" min={1} placeholder="Nhập ngày" />
                    </Form.Item>
                    <Form.Item
                      {...restField}
                    ...
                    >
                      <Input type="time" placeholder="Chọn giờ bắt đầu" />
                    </Form.Item>
                    <Form.Item
                      {...restField}
                      name={[name, 'end_time']}
                      ...
                      rules={[
                        ...
                        ({ getFieldValue }) => ({
                          validator(_, value) {
                            const startTime = getFieldValue(['schedules', name, 'start_time']);
                            if (!startTime || !value) {
                              return Promise.resolve();
                            }
                            if (value <= startTime) {
                    ...
                    >
                      <Input type="time" placeholder="Chọn giờ kết thúc" />
                    </Form.Item>
                    <Form.Item
                      {...restField}
                      name={[name, 'title']}
                    ...
                    <Form.Item
                      {...restField}
                      name={[name, 'activity_description']}
                      label="Mô tả hoạt động"
                    >
                      <TextArea rows={2} placeholder="Mô tả hoạt động" />
                    </Form.Item>
                    <Form.Item
                      {...restField}
                      ...
                      label="Điểm đến"
                    >
                      <Select allowClear placeholder="Chọn điểm đến">
                        {destinations.map((d) => (
                          <Select.Option key={d.destination_id} value={d.destination_id}>
                            {d.name}
                          </Select.Option>
          ...
          <Form.Item>
            <Space>
              <Button
                ...
                htmlType="submit"
                loading={loading}
                disabled={categories.length === 0 || destinations.length === 0}
              >
                Tạo tour
              </Button>
